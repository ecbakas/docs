...
| A38 | Complete a claim deferred through login, once authenticated | Traveller, logged out then authenticated | Log in to claim, then the `(auth)` mount effect — no second press | `super-app` | `/tag-preview`, `/(auth)` | `super-app/src/screens/shared/TagPreviewScreen.tsx:185`–`188` (the Login-to-claim control; its handler `loginToClaim` is at `:156`), `super-app/src/hooks/useResumePendingScan.tsx:31` | `postTagTravellerSelfAssign` — `super-app/src/actions/TagService/actions.ts:76` | `client.tag.postApiTagServiceTagTravellerSelfAssign` | POST /api/tag-service/tag/traveller-self-assign | TagService.Tags, TagService.Tags.TravellerSelfAssign | 4 | T2, T3 |
...
| A61 | Print the tag through the report service | Merchant, Refund Point, Customs, Admin | Print tag button in the tag detail footer | `web-app/apps/web` | `/operations/tax-free-tags/[tagId]` | `web-app/apps/web/src/app/[lang]/(main)/(unirefund)/operations/tax-free-tags/[tagId]/_components/tag-actions.tsx:107` | `getReportsSynchronouslyByEntityApi` — `web-app/packages/actions/unirefund/ReportService/actions.ts:17`, called at `web-app/apps/web/src/app/[lang]/(main)/(unirefund)/operations/tax-free-tags/[tagId]/_components/tag-actions.tsx:115` | `client.report.getApiReportServiceReportsSynchronouslyByEntity` | GET /api/report-service/reports/synchronously/by-entity | ReportService.Reports, ReportService.Reports.CreateSynchronously | — | — |
...
SSR's only in-app **barcode** cameras are the boarding-pass scanner (A89), the
expired-QR rescan modal (A92) and the claim-tag modal's scan tab (A93) — the
three `BarcodeCameraScanner` mounts in `apps/ssr/src`, enumerated rather than
assumed. Two further cameras exist in the app and read no code this guide is
about: a `CreditCardScanner` in the add-card dialog
(`web-app/apps/ssr/src/app/[lang]/(main)/account/cards/_components/add-card-dialog.tsx:132`)
and one in the card-scanner demo
(`web-app/apps/ssr/src/app/[lang]/(public)/card-demo/_components/card-scanner-demo-client.tsx:68`).
Neither takes a row: a payment card is not a Unirefund QR. Everything else
