...
Two are new to this pass and were not in the running notes. `F5` is the larger: the
...
them. The other new row is `F22`, closed as a causal chain and left open as a catalogue
question.

`F7` and `F19` are narrower than first recorded: `TagService.Tags.View` is a real
permission rather than an invented identifier, and the `ssr-dev` host is a fallback
rather than a hardcoded one. No Finding on the running list was dropped as unfounded.
