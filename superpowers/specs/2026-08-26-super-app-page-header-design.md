...
4. **The `px-4` inset cannot be judged in this round.** The attached device is **800dp wide** (1200 / 1.5) — a tablet. An 8px inset shift is negligible at 800dp and is ~4.4% of screen width at 360dp, so the tablet cannot answer the question the checkpoint existed to answer. The `Pixel_9_Pro` AVD is unavailable (user-confirmed), and a compact device will be arranged after this round.

   Consequence: **the `px-4` unify lands unvalidated at phone width.** It therefore goes in its own commit, separate from the back-button work, so it can be reverted without disturbing the fix if the compact device later says it is wrong. Screenshots of the affected modal screens are captured on the tablet anyway — not as a verdict, but as a before/after record for whoever reviews on the phone.
...
- **The `px-4` unify is the only change that touches screens with no back-button problem, and it is the one change no available device can validate.** All eighteen `ModalTemplate` screens get 8px wider content, judged on an 800dp tablet where the shift is barely perceptible. It is cosmetic, carries the most regression risk here, and ships on reasoning rather than evidence. Isolated in its own commit for exactly that reason — see Verification 4.
