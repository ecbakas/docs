...
Implemented and merged to `main` as `3e8bdc113` — 11 task commits plus one
final-review fix commit, branched from `origin/main` (dfd2ed442).

Final state: `pnpm --filter ssr type-check` **clean, exit 0**;
`pnpm --filter ssr lint` clean; `i18n:unused --app=ssr` 0 unreachable keys;
`find-missing-i18n.mjs --app=ssr` 0 missing keys.

**A baseline correction worth recording.** Throughout execution the gate was
stated as "clean apart from one pre-existing `logo.tsx` SVG error". That error was
never pre-existing: `apps/ssr/next-env.d.ts` is gitignored and only generated by
`next dev`/`next build`, and it carries the `next/image-types/global` reference
that declares `*.svg` modules. A fresh worktree has neither. Running the dev server
generated it and the error vanished. Ten subagents were told to tolerate one type
error — harmless here, since there was exactly one, but it could have masked a real
one. **In a fresh worktree, run the dev server once before measuring a typecheck
baseline.**

### Open items

Ranked. The first is a decision, not a defect.

1. **`isPending` is probably inert, and nothing guards double-submit.** Both
   `cards-view.tsx` and `add-bank-dialog.tsx` use
   `startTransition(() => { void promise.then(...) })`. The callback returns
   `undefined`, so React 19 does not await it — the transition ends in the same
   tick. If so, every `disabled` prop is permanently false, the add-bank button
   never shows "Adding…", and it can be double-clicked into duplicate submissions.
   **This is inherited verbatim from the pre-existing `add-card-dialog.tsx`**, so
   it is not a regression and fixing it changes behaviour shared with the card
   flow — which is why it was deliberately left out of the fix wave. Needs a
   decision, then probably one change applied to both dialogs.
2. **`CardRow` can still overflow for one combination** — a card that is *both*
   expired *and* has a very long nickname. `buttonVariants`' base `shrink-0` is
   never overridden, so the nickname button and the expired badge both freeze at
   their clamped widths. Severity is much reduced from the original defect and
   `BankRow` is unaffected. Parked as cosmetic.
3. **`normalizeIban` strips only `\s`.** A pasted IBAN containing hyphens, dots or
   a zero-width space is silently rejected as invalid with no recovery path. The
   module is a deliberate verbatim port from super-app, so diverging is a decision;
   the fix would be `replace(/[\s.​-]/g, "")` in both clients.
4. **BIC is unvalidated** despite the field table specifying 8–11 characters — a
   short BIC costs a round trip and surfaces a raw backend message.
5. **`formatIban` is dead code.** Its obvious consumer — grouping the IBAN input as
   the user types, mirroring `formatCardNumber` in the card dialog — was never
   wired up, and the IBAN input has no `maxLength`.
6. **Four Turkish tone inconsistencies** against neighbouring `Account.Cards.*`
   strings (a dropped "başarıyla", "ad" vs "etiket", "kaldırılacak" vs
   "kaldırılacaktır"). Worth one pass by a Turkish speaker.
7. Cosmetic: the two Add buttons use different variants (filled vs outline), and
   `PartitionableToken`'s doc comment claims `id` is read by a function that never
   reads it.
