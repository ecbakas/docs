...
| 34  | The generated client's create comment matches the endpoint it calls              | unirefund-web             | `web-app`'s `by-sticker-line` comment says an unallocated header is rejected; `super-app`'s newer copy of the same comment, and `merchant-info` in both trees, say the create allocates. The apps are right and this copy is stale. Regenerate. Guide `F5` |
