...
Numbers `#30`–`#53`, taken by the defects the survey behind [`qr/README.md`](qr/README.md) turned up. Each is a **new** capability taking the next free number, per the rule at the top of this catalogue; nothing above is renumbered and no decision above is changed. `#52` and `#53` were added by the guide's final whole-branch review, which found two defects the chapters had described and promised to route but had not: guide `F27` and `F28`.
...
These 24 arrive **undecided**. The closing line of the 2026-07-29 decisions — that nothing in the catalogue is undecided — was true of the catalogue as it stood that day and is left as written; it does not extend to this section.
...
| 38  | A gate checks the grant its destination uses                                     | unirefund-web/apps/web    | `scan-sticker`'s `canViewTag` checks `Tags.ViewSummary` before navigating to a tag detail page whose own read needs `Tags.Detail`; `ViewSummary` belongs to an unrelated aggregate financial/risk endpoint that no flow in the guide reaches. The *nav* gate is not the defect — the tax-free-tags page really does call the summary endpoint. Guide `F9` |
...
| 50  | `GET /tag` honours its risk filters or rejects them                              | backend                   | Accepted and silently dropped at 200 **when the caller lacks `TagService.TagRisks.FilterByRisk`** — documented as intentional, so client integrations stay error-free across deployments. The failure mode that leaves: a caller that sends them anyway gets an unfiltered page indistinguishable from a filtered one. Guide `F25` |
...
| 52  | A claim is only offered on a tag that can actually be claimed                    | unirefund-web/apps/ssr    | The validate-results claim modal performs **no** draft/issued check — a scanned tag goes straight to a Claim button and a typed tag number is posted with no lookup at all — relying on the endpoint's rejection, shown as a generic toast. mobile's equivalent modal pre-checks and says which case it is; so does `apps/ssr`'s own `/tag/[slug]` page. Guide `F27` |
| 53  | "No tag on this sticker yet" is a message, not a generic failure                 | unirefund-web/apps/ssr    | mobile has dedicated copy telling the traveller to ask the store; `apps/ssr` falls through to the same `LookupFailed` every other lookup failure uses. Same scan, same outcome, one app says what to do about it. Guide `F28` |
